...
For the Workshop, we will conduct data exploration on a smaller subset of the production data rather than the full tables that hold data going back to 2021. We will pull the "*_deckhand" versions of the same data frames we wrote in the previous exercise, which reside in the "deckhand" schema.  

You'll notice we're also using the `collect()`

```{r}
#| label: read in raw data

vesselinfo_raw <- dplyr::tbl(con, DBI::Id("deckhand", "vesselinfo_raw_deckhand"))

vesselhistory_raw <- dplyr::tbl(con, DBI::Id("deckhand", "vesselhistory_raw_deckhand"))

weather_terminal_history_raw <- dplyr::tbl(con, DBI::Id("deckhand", "weather_terminal_history_raw_deckhand"))
