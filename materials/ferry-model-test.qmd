...
ferry_model_data <- ferry_data |> 
  ...
  select(departing, closest_hour, date, precipitation, weather_code, cloud_cover_low, wind_speed_10m, wind_gusts_10m, delay) |> 
  
  # Take log of depart. Need to add constant so all values are positive
  #mutate(delay = log(delay + abs(min(delay)) + 1)) |> 
...
v_model_monitor <- vetiver_pin_read(board, "ryan/ferry_model", version = "557746")

new_ferry_data_metrics <-
    augment(v_model_monitor, new_data = monitoring_data) |> 
