...
Navigate back to your `.Renviron` file and add the following line. It's always a good idea to add an additional blank line at the end of your `.Renviron` file.
...
ferry_weather <- dplyr::tbl(con, 
           DBI::Id(schema = "deckhand", name = df_name)) |> 
  collect()
...
board <- board_connect(auth = "manual", 
                       server = "https://pub.ferryland.posit.team", 
...
vetiver_deploy_rsconnect(board = board, name = "ryjohnson09/ferry_model")
...
#  Replace "ryan" with your username 
pin_versions(board = board, name = "ryjohnson09/ferry_model")
```
...
# Create Vetiver Model
v_model_xgb <- vetiver_model(ferry_fit_xgb, "ferry_model")
...
v_model_monitor <- vetiver_pin_read(board, "ryjohnson09/ferry_model", version = "49")
