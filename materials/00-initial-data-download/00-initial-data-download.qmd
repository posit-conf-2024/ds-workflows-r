...
vesselinfo_raw <- vesselinfo_raw |> unnest(Class) |> clean_names()

```

### Make a helper function to download historical vessel data

This is nicer than a raw API call because there we include helpful print
statements for status.
...
get_terminal_weather_history <- function(terminal, start_date, end_date, pause=0) {
  ...
  lat <- terminallocations |> filter(terminal_name == terminal) |> pull(latitude)
  long <- terminallocations |> filter(terminal_name == terminal) |> pull(longitude)
...
terminallocations <- terminallocations_raw |> 
  select(TerminalID, TerminalName, Latitude, Longitude) |> clean_names()
