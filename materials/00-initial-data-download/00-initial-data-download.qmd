...
my_df_name <- glue("weather_terminal_history_raw_{config::get('suffix', config=env)}")

# con <- dbConnect(RPostgres::Postgres(), 
#                  host = Sys.getenv("DATABASE_HOST"), 
#                  port = "5432", 
#                  dbname = Sys.getenv("DATABASE_NAME_R"), 
#                  user = config::get("user", config=env), 
#                  password = config::get("password", config=env))
con <- dbConnect(
  odbc::odbc(),
  Driver      = "postgresql",
  ...
  UID         = config::get("user", config=env),
  PWD         = config::get("password", config=env),
  timeout     = 10
)
con
...
my_df_name <- glue("vesselhistory_raw_{config::get('suffix', config=env)}")
