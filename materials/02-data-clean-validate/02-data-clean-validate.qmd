...
Notice that in the quarto code chunks, we have set `#| eval: !expr 'stop_alert != TRUE'` to prevent the code from running if there is a data integrity issue. This will save computation time by not bothering to render code chunks with questionable data.
...
Pointblank has identified all of the rows of `vesselhistory_w_weather` that
