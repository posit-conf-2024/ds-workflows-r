...
stop_notice != TRUE | !Sys.getenv("CONDITION_OVERRIDE") %in% c(1:3)
...
The following validation(s) triggered a warning flag:
See attached extract files for failing records.
...
Review the validation results below for more details. 
...
The ferry data for modeling has been cleaned, validated, and `{r} nrow(modeldata_validated)` records were written to the database.
