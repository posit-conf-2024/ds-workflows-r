...
main_agent <- create_agent(vesselhistory_w_weather) |> 
  ...
  col_vals_lte(columns = date, today()-30,
...
# summary_table <- modeldata_validated |>
# select(route, vessel,scheduled_depart, delay) |>
# group_by(route, vessel) |>
# mutate(avg_delay = mean(delay)) |>
# ungroup() |>
# pivot_wider(names_from = scheduled_depart, values_from = delay ) |>
# gt(rowname_col = "vessel", groupname_col = "route") |>
# cols_nanoplot(
...
# new_col_label = "delay"
# ) |>
# fmt_number(columns = avg_delay, decimal = 0) |>
# # opt_all_caps("row_groups") |>
# summary_rows(
# groups = everything(),
# columns = avg_delay,
# funs = list(
...
warnings_table <- data.frame(step, desc) |> gt() |> cols_label(step="Validation Step",desc="Description")
...
purrr::map2(failures, step, ~{write_csv(.x, file = glue("extracts_step_{.y}.csv"))})

filenames_yaml <- glue(" - extracts_step_{step}.csv") |> str_c(collapse = "\n")

```

Add the attachments as metadata entries
```{r}
...
if (condition == 1){
glue("Data integrity validation failed. Data processing for the model training did not proceed. An email notification of the failure was sent.")
} 
...
glue("Vessel history and weather data was validated and sundered, however a warning was triggered due to one or more validation steps exceeding the threshold for allowed failures.
