...
-   Write the raw data to a database, using `{config}` to control whether we are writing data for production or for development and testing 
...
Our database has two schemas: "production" and "deckhand." For the purposes of the Workshop, each participant will be writing to the "deckhand" schema but the production project will be written to the "production" schema. 

```{r}
#| label: database write

my_username <- "_____" #Fill in your username

if(Sys.getenv("DATABASE_SCHEMA") == "production"){
  df_suffix <- "production"
} else{
  df_suffix <- my_username
}
...
Rather than manually repeating the API call for each vessel though, we'll use a function we wrote into the `ferryfairy` package that iterates over the list of vessel names.
...
Again, to simplify things, we use a function in `ferryfairy` to retrieve latitude and longitudes for each terminal.
...
We'll use a helper function from `{ferryfairy}` for brevity.

```{r}
#| label: write remaining data to database

write_df_to_db(vesselhistory_raw, paste0("vesselhistory_raw_",my_username), con, schema = Sys.getenv("DATABASE_SCHEMA"), append = TRUE)

write_df_to_db(terminallocations, paste0("terminallocations_",my_username), con, schema = Sys.getenv("DATABASE_SCHEMA"), overwrite = TRUE)

write_df_to_db(weather_terminal_history_raw, paste0("weather_terminal_history_raw_",my_username), con, schema = Sys.getenv("DATABASE_SCHEMA"), append = TRUE)
...
## Task 7 - Send an email alert if something is fishy 🐟
