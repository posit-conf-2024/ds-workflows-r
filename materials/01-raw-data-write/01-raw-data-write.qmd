...
-   Use push-button publishing to publish this document, with source code, to Connect, but remember, _it is not good practice to bundle your `.Renviron` file with your deployment. Once the code is deployed to Connect, we will need to input our `WSDOT_API_KEY` in the Connect "Vars" pane. Or  we can publish the environment variable value directly using `rsconnect::deployApp(appFiles = c("01-raw-data-write.qmd", "mode_switch.png"), envVars = c("WSDOT_ACCESS_CODE"))`
