...
function header_column_narrow_dark(c)
    local result = pandoc.List({})
    local title = pandoc.utils.stringify(c[1].content)
    result:insert(pandoc.RawBlock("html", "<h2 style='color: #F0F0F0; margin-right:150px;'>" .. title .. "</h2>"))
    return result
end
...
function bracket_content(c)
    local result = pandoc.List({})
    local title = pandoc.utils.stringify(c[1].content)
    result:insert(pandoc.RawBlock("html", "<br><h2 style='margin-top:10px;'>" .. title .. "</h2>"))
    result:extend(c)
    result:remove(2)
    return result
end
...
for i, v in ipairs({
    ...
    { image = "brackets-green",
      dark = true,
      widths = { "100%" },
      columns = { bracket_content },
    },
    { image = "brackets-dark-teal",
    dark = true,
    widths = { "100%" },
    columns = { bracket_content },
  },
    ...
    { image = "title-slide-dark",
      dark = true,
      widths = { "100%" },
      columns = { content_column },
    },
    { image = "title-slide-light",
      widths = { "100%" },
      columns = { content_column },
    },
    { image = "thank-you",
