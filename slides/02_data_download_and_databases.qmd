...
- 🛑 We will work through **Task 1 - Task 3** only 
...
- 🛑 We will work through **Task 4 - Task 5** only 

:::

## A brief {{< bi sign-turn-right-fill color=orange >}} on managing environments

🧰 `{config}` can help you manage your development, test, and production environments without modifying your code
...
🧰 Similarly, update environment variables to change behavior in development and production 
```{.r}
...
One Quarto document produces the rendered report and an [optional]{.positburg} email ✨
...
🧰 Include a conditional value that expresses `TRUE`|`FALSE` within a div called `::: {email-scheduled}` to send or suppress the email.
